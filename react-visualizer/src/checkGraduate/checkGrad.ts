...
export function checkGraduate(grade, req, callback) {
  for (let rishu = 0; rishu < 2; rishu++)
    for (let hishu = 0; hishu < 2; hishu++)
      for (const k1 in req) {
        req[k1] = genNow(req[k1], rishu);
        for (const k2 in req[k1]["leaf"]) {
          req[k1]["leaf"][k2] = genNow(req[k1]["leaf"][k2], rishu);
          for (const k3 in req[k1]["leaf"][k2]["leaf"]) {
            ...
            for (const k4 in req[k1]["leaf"][k2]["leaf"][k3]["leaf"]) {
              if (
                ...
                for (const k5 in req[k1]["leaf"][k2]["leaf"][k3]["leaf"][k4][
                  "leaf"
                ]) {
                  ...
                  req[k1]["leaf"][k2]["leaf"][k3]["leaf"][k4][now] += Math.min(
                    rishu *
                      req[k1]["leaf"][k2]["leaf"][k3]["leaf"][k4]["leaf"][k5][
                        now
                      ],
                ...
                req[k1]["leaf"][k2]["leaf"][k3][now] += Math.min(
          ...
          req[k1][feature] += hishu * rishu * req[k1]["leaf"][k2][feature];
        }
      }
...
function checkGet(grade, req, rishu) {
  ...
  for (const key in grade) {
    ...
    if (flag) {
      if (
        grade[key]["used"] === false &&
        grade[key]["used_rishu"] === false &&
        req[now] < req[max]
      ) {
