...
function convertArray(data) {
  ...
  for (let i = 0; i < dataString.length; i++)
    dataArray[i] = dataString[i].replace(/^"/i, '').replace(/"$/i, '').split('","');
...
function genGradeObj(array: String[][]) {
  ...
  array.forEach(function (ar) {
    if (ar.length === 11 && ar[0] !== "学籍番号") { res.push(parseGrade(ar)) }
  });
  return res;
}
...
function parseGrade(array: String[]) {
  ...
  res["grade"] = array[7]
  res["can_use"] = array[7] === "P" || array[7] === "A+" || array[7] === "A" || array[7] === "B" || array[7] === "C" || array[7] === "認" // || (array[7] === "履修中" && args.expect)
  res["used"] = false
